    ...
    for M in 10000 50000; do
      ...
      for EST_FREQ in 0.25 "NONE"; do
        ...
        if [ "$EST_FREQ" == "NONE" ]; then
          ...
          start_pl=`date +%s`
          perl pairwiseDistances.pl -n $N_CPU simulated.fasta > results_pl.txt
          end_pl=`date +%s`
          runtime_pl=$((end_pl-start_pl))

          # Test speed of python program
          start_py=`date +%s`
          python3 calc_hamming_dist.py -n $N_CPU simulated.fasta > results_py.txt
          end_py=`date +%s`
          runtime_py=$((end_py-start_py))
        else
          # Test speed of perl program
          start_pl=`date +%s`
          perl pairwiseDistances.pl -n $N_CPU -e -s $EST_FREQ simulated.fasta > results_pl.txt
          end_pl=`date +%s`
          runtime_pl=$((end_pl-start_pl))

          # Test speed of python program
          start_py=`date +%s`
          python3 calc_hamming_dist.py -n $N_CPU -e -s $EST_FREQ simulated.fasta > results_py.txt
          end_py=`date +%s`
          runtime_py=$((end_py-start_py))
        fi
